...
package com.stepin2it.stepin2it.customlayouts;

import java.util.HashSet;
import java.util.Set;

import com.stepin2it.stepin2it.R;
import com.stepin2it.stepin2it.FeaturesConfiguration;
...
public class SemiCircleLayout extends ViewGroup {
	...
	private static final int TOTALDEGREES = 180;
	...
	public SemiCircleLayout(Context context, AttributeSet attrs) {
		...
		try {
			...
			mAngleOffset = a.getFloat(R.styleable.CircleLayout_angleOffset, 180f);
			mDividerWidth = a.getDimensionPixelSize(R.styleable.CircleLayout_dividerWidth, 1);
			mInnerRadius = a.getDimensionPixelSize(R.styleable.CircleLayout_innerRadius, FeaturesConfiguration.getCustomViewRadius());
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for(int i=0; i<childs; i++) {
			...
			final float angle = TOTALDEGREES/totalWeight * lp.weight;
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if(action == MotionEvent.ACTION_DOWN) {
			...
			for(int i=0; i<childs; i++) {
				...
				float startAngle = lp.startAngle % TOTALDEGREES;
				float endAngle = lp.endAngle % TOTALDEGREES;
				float touchAngle = angle;
				
				if(startAngle > endAngle) {
					if(touchAngle < startAngle && touchAngle < endAngle) {
						touchAngle += TOTALDEGREES;
					}
